              ...
              {appointmentOccurrences.map((appointment) => (
                ...
                >
                  <div className="flex items-start justify-between gap-3">
                    ...
                    {appointment.repeat && !appointment.endsAt ? (
                      <span
                        aria-label="Recurring appointment has no end date"
                        className="group relative flex h-5 w-5 shrink-0 items-center justify-center rounded-full border border-amber-300 bg-amber-50 text-xs font-semibold text-amber-800"
                      >
                        i
                        <span className="pointer-events-none absolute bottom-7 right-0 z-10 hidden w-64 rounded-md border border-slate-200 bg-white px-3 py-2 text-left text-xs font-normal text-slate-700 shadow-sm group-hover:block">
                          This recurring appointment has no end date and will
                          keep appearing in future schedules until an end date is
                          added in the EMR.
                        </span>
                      </span>
                    ) : null}
